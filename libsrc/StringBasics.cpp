...
#ifdef __GNUC__
   String::operator long double () const { return strtold(buffer, NULL); }
#else
   #ifdef __BORLANDC__
      String::operator long double () const    { return _strtold(buffer, NULL); }
   #else
      String::operator long double () const    { return atof(buffer); }
   #endif
#endif
