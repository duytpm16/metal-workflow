...
void ClearFilters()
   {
   printf("# Cleared user defined filters ...\n");
...
void ProcessFile(String & filename, FileSummary * history)
   ...
   while (!ifeof(f))
      ...
      if (marker < 0)
         {
         if (markerLookup.Entries() == 0 && customColumns.Length())
            custom = new Vector[customColumns.Length()];

         markerLookup.SetInteger(tokens[markerColumn], marker = markerLookup.Entries());
         statistics.Push(0);
   ...
   if (genomicControl)
      {
      if (chiSquareds.Length() == 0 && genomicControlLambda == 0.0)
         {
...
void ShowHelp(bool startup)
   ...
   printf("# This program faciliates meta-analysis of genome-wide association studies.\n"
          ...
          "#   GENOMICCONTROL   [ON|OFF|VALUE]              (%s = %s)\n"
          "#\n"
...
void RunScript(FILE * file)
   ...
   while (!feof(file))
      ...
      if (tokens[0].MatchesBeginningOf("FREQLABEL") == 0 && tokens[0].Length() > 1)
         ...
         if (!averageFrequencies) printf("## If you want frequencies to averaged, issue the 'AVERAGEFREQ ON' command\n");
         continue;
         }
